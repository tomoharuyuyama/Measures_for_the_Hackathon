...
<section class="mt-10 px-10">
  @foreach ($tasks as $task)
    <form action="{{route('mypage.done', $task->id)}}" method="POST">
      @csrf
      @method('PUT')
      ...
      <div class="flex mb-5 bg-white h-20 justify-around rounded-lg shadow-sm hover:shadow-md transition-shadow px-2">
        ...
        <button class="w-20 h-7 my-auto text-xs rounded-2xl bg-gradient-to-r from-blue-700 to-blue-500 text-white">完了</button>
